...
############################################
logdir="/home/mythtv/transcodelogs" #change to your needs for logs
...
lang2_name="qaa" #This is the label of the 2nd audio stream inside the final video file
...
subtitle="$(echo "$6" | sed 's/\s/-/g')"
title="$(echo "$5" | sed 's/\s/-/g')"
category="$(echo "$7" | sed 's/\s/-/g')"
season="$(echo "$8")" 
episode="$(echo "$9")" 
chanid="$3"
starttime="$4"
...
chmod a+rw "$logfile"
filename="${title}_s${season}e${episode}_${subtitle}.m4v" # can be customized
...
if [ -f "$outdir/$filename" ]
 ...
 then
 filename="${filename}-${scriptstarttime}.m4v"
fi
...
#HandBrakeCLI -q 19.0 -e x264 -r 25 -a "$audiotacks" -A "$audioname" -E "$audiocodec" -B "$audiobitrate" -6 "$audiodownmix" -f mp4 --crop 0:0:0:0 -d -m -x b-adapt=2:rc-lookahead=50:ref=3:bframes=3:me=umh:subme=8:trellis=1:merange=20:direct=auto -i "$mythrecordingsdir/$file" -o "$outfile"  --optimize 2>> "$logfile"
	#rsync -av $mythrecordingsdir/$file macmini:
	HandBrakeCLI -Z 'Apple 2160p60 4K HEVC Surround' -i $file -o $filename  >> "$logfile"
  if [ $? != 0 ]
...
#HandBrakeCLI -q 19.0 -e x264 -r 25 -a "$audiotacks" -A "$audioname" -E "$audiocodec" -B "$audiobitrate" -6 "$audiodownmix" -f mp4 --crop 0:0:0:0 -d -m -x b-adapt=2:rc-lookahead=50:ref=3:bframes=3:me=umh:subme=8:trellis=1:merange=20:direct=auto -i "$mythrecordingsdir/$file" -o "$outfile"  --optimize 2>> "$logfile"
	HandBrakeCLI -Z "Apple 1080p30 Surround" -i "$mythrecordingsdir/$file" -o "$outfile"  >> "$logfile"
      if [ $? != 0 ]
	# There were errors in the Handbrake Run. 
	then
	scriptstoptime=$(date +%F-%H%M%S)
	...
	echo "Transcode Run successful." >> "$logfile"     
      ...
      SDCCMDLINE='/usr/bin/HandBrakeCLI -Z "Apple 2160p60 4K HEVC Surround" -i "$mythrecordingsdir/$file" -o "$outfile" >> "$logfile"' 
...
rsync -av $outfile macmini: >> "$logfile"
exit 0 
